...
<div class="main">
    ...
    foreach($_POST['genre'] as $check) {
		if(!empty($_POST['language'])){
         ...
         $movies = @mysql_query("SELECT * FROM poster NATURAL JOIN movies where type='small' and genre='$check' and language='$lang' ") or die(mysql_error());
		$num=@mysql_num_rows($movies);
	if($num==0){
		?>
	...
	<?php
		...
		$movies = @mysql_query("SELECT * FROM poster NATURAL JOIN movies where type='small' and genre='$check' ") or die(mysql_error());
		$num=@mysql_num_rows($movies);
	if($num==0){
		?>
		<h3>No Results Found!!</h3>
	...
	<?php
			...
			foreach($_POST['language'] as $lang) {
         $movies = @mysql_query("SELECT * FROM poster NATURAL JOIN movies where type='small' and language='$lang' ") or die(mysql_error());
		$num=@mysql_num_rows($movies);
	if($num==0){
		?>
		<h3>No Results Found!!</h3>
